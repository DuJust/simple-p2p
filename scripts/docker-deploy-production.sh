...
echo "Running a new ${APP_NAME} container..."
docker run --name ${APP_NAME} --link ${DB_HOST}:${DB_HOST} -e DB_USER=${DB_USER} -e DB_PASSWORD=${DB_PASSWORD} -e DB_HOST=${DB_HOST} -e DB_NAME=${APP_NAME} -e SECRET_KEY_BASE=${SECRET_KEY_BASE} -e RAILS_ENV=production -d ${APP_NAME}
